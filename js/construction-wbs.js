...
function paint(){const top=host.querySelector('.wbs-tree-scroll')?.scrollTop||0;host.innerHTML=`<div class="wbs-layout"><aside class="wbs-tree"><header><strong>项目文件</strong><button class="text-button" data-wbs-action="collapse">收起</button></header><label class="wbs-tree-search">${icon('search')}<input id="wbsTreeSearch" aria-label="搜索 WBS 节点" value="${esc(state.treeQuery)}" placeholder="查找编码或工作包"></label><div class="wbs-tree-scroll">${tree(nodes[0])||'<p class="build-empty">没有匹配节点</p>'}</div></aside><section class="wbs-main">${main()}</section></div>`;bindInputs();host.querySelector('.wbs-tree-scroll').scrollTop=top;const selected=filtered().find(d=>d.id===state.selected);if(selected)docPanel(selected,false);else state.selected=null;}
